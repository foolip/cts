...
g.test('OOB color attachment indices are handled').params([{
...
g.test('check layer count for color or depth stencil').params([{
  arrayLayerCount: 5,
  baseArrayLayer: 0,
  _success: false
...
g.test('check mip level count for color or depth stencil').params([{
  mipLevelCount: 2,
  baseMipLevel: 0,
  _success: false
...
g.test('it is invalid to set resolve target if color attachment is non multisampled').fn(async t => {
  const colorTexture = t.createTexture({
    sampleCount: 1
  });
...
g.test('it is invalid to use a resolve target with array layer count greater than 1').fn(async t => {
  const multisampledColorTexture = t.createTexture({
    sampleCount: 4
  });
  const resolveTargetTexture = t.createTexture({
...
g.test('it is invalid to use a resolve target with mipmap level count greater than 1').fn(async t => {
  const multisampledColorTexture = t.createTexture({
    sampleCount: 4
  });
  const resolveTargetTexture = t.createTexture({
...
g.test('use of multisampled attachment and non multisampled resolve target is allowed').fn(async t => {
  const multisampledColorTexture = t.createTexture({
    sampleCount: 4
  });
  const resolveTargetTexture = t.createTexture({
...
g.test('use a resolve target in a format different than the attachment is not allowed').fn(async t => {
  const multisampledColorTexture = t.createTexture({
    sampleCount: 4
  });
  const resolveTargetTexture = t.createTexture({
