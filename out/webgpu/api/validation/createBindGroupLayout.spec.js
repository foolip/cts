...
g.test('number of dynamic buffers exceeds the maximum value').params([{
  type: C.BindingType.StorageBuffer,
...
g.test('dynamic set to true is allowed only for buffers').params(poptions('type', kBindingTypes)).fn(async t => {
  const {
    type
  } = t.params;
...
g.test('max resources per stage/in bind group layout').params(kCasesForMaxResourcesPerStageTests).fn(async t => {
  const {
    maxedType,
    extraType,
    maxedVisibility,
...
g.test('max resources per stage/in pipeline layout').params(kCasesForMaxResourcesPerStageTests).fn(async t => {
  const {
    maxedType,
    extraType,
    maxedVisibility,
