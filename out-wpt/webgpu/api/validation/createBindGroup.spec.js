...
g.test('buffer binding must contain exactly one buffer of its type').params(params().combine(poptions('bindingType', kBindingTypes)).combine(poptions('resourceType', kBindableResources))).fn(t => {
  const {
    bindingType,
    resourceType
  } = t.params;
...
g.test('texture binding must have correct usage').params(params().combine(poptions('type', kTextureBindingTypes)).combine(poptions('usage', kTextureUsages))).fn(async t => {
  const {
    type,
    usage
  } = t.params;
...
g.test('texture must have correct component type').params(poptions('textureComponentType', [C.TextureComponentType.Float, C.TextureComponentType.Sint, C.TextureComponentType.Uint])).fn(async t => {
  const {
...
g.test('buffer offset and size for bind groups match').params([{
  offset: 0,
  size: 512,
  _success: true
}, // offset 0 is valid
