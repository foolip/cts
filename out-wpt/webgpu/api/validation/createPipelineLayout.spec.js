...
g.test('number of dynamic buffers exceeds the maximum value').params(params().combine(poptions('visibility', [0, 2, 4, 6])).combine(poptions('type', [C.BindingType.UniformBuffer, C.BindingType.StorageBuffer, C.BindingType.ReadonlyStorageBuffer]))).fn(async t => {
  const {
    type,
    visibility
  } = t.params;
...
g.test('visibility and dynamic offsets').params(params().combine(poptions('type', kBindingTypes)).combine(pbool('hasDynamicOffset')).combine(poptions('visibility', kShaderStageCombinations))).fn(t => {
  const {
    type,
    hasDynamicOffset,
    visibility
